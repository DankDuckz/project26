...
//Call functions to detect collision for player and computer
  ...
  fill("blue")
  textSize(30)
  text("PLAYER SCORE:" + plrScore,200,100)
  fill("red")
  text("COMPUTER SCORE:" + plrScore,width-225,100)
...
// Write code to detect collision between player arrow and opponent
  for (var i = 0;i < playerArrows.length;i++) {
    var bCollision = Matter.SAT.collides(playerArrows[i].body,computerBase.body)
    var aCollision = Matter.SAT.collides(playerArrows[i].body,computerArcher.body)
    var cCollision = Matter.SAT.collides(playerArrows[i].body,computer.body)
    var plrPos =  playerArrows[i].body.position

    if (cCollision.collided || aCollision.collided) {
      World.remove(world,playerArrows[i])
      playerArrows.splice(i,1)
      console.log("collided with comp")
      plrScore += 1
    }
    else if(bCollision.collided) {
      World.remove(world,playerArrows[i])
      playerArrows.splice(i,1)
      console.log("collided with compBase")
    }
    else if (plrPos.x > width || plrPos.y > height || plrPos.x < 0 || plrPos.y < 0) {
...
function handleComputerArrowCollision() {
  ...
  for (var i = 0;i < computerArrows.length;i++) {
    var bCollision = Matter.SAT.collides(computerArrows[i].body,playerBase.body)
    var aCollision = Matter.SAT.collides(computerArrows[i].body,playerArcher.body)
    var pCollision = Matter.SAT.collides(computerArrows[i].body,player.body)
    var compPos =  computerArrows[i].body.position

    if (pCollision.collided || aCollision.collided) {
      World.remove(world,computerArrows[i])
      computerArrows.splice(i,1)
      console.log("collided with plr")
      compScore += 1
    }
    else if(bCollision.collided) {
      World.remove(world,computerArrows[i])
      computerArrows.splice(i,1)
      console.log("collided with plrBase")
    }
    else if (compPos.x > width || compPos.y > height || compPos.x < 0 || compPos.y < 0) {
